...
1. ~~Волна S плана ARMS~~ — ЗАКРЫТА 05.09 (PR #271: deck `/skills`, cron-допуск llm, мета-навыки). Следующее по порядку:
   первая сессия Venture Factory (§9 п. 5), затем волна M.
2. ~~Волна M~~ — ЗАКРЫТА 06.09 (ветка `feat/wave-m-docs-brain`, решение
   `docs/decisions/2026-09-06-wave-m-docs-brain.md`): роутеры направлений, зеркала движка под тестом
   дрейфа (не 51 битая ссылка, а закрыто тестом), панель «Документы» (`/docs`), граф «Мозг» (`/brain`);
   `PROTOCOL.md`/`MYDON_AGENT_BUILDER.md` нигде не найдены — заменены `_template/` +
   `direction-package.md` + восстановленный `run-log.md`; RAG не включён (решение владельца). Дальше —
   волна R (доска срабатываний, хуки, плейбэк).
