...
vi.mock("../../../components/agent-editor", () => ({
  AgentEditor: ({ autonomyMax }: { autonomyMax: string | null }) => (
    <div>Настройки агента · порог {autonomyMax ?? "не прочитан"}</div>
  ),
}));
...
describe("Карточка агента: порог автономии", () => {
  ...
  it("конфиг не прочитался — редактор получает «не прочитан», а не выдуманный T0", async () => {
    ...
    expect(screen.getByText("Настройки агента · порог не прочитан")).toBeVisible();
  });
});
