  ...
  return (
    <>
      ...
      {/* `<select>` не шлёт change на уже выбранное значение — повтор нужен
          ...
          ГЛАГОЛ ПЕРВЫМ, причина второй. Кнопка режется многоточием
          (`max-width: 180px` в `globals.css`), а «Не удалось сохранить тему ·
          повторить» ≈ 212px: в прежнем порядке на 390px глаголу доставалось
          ~46px и владелец видел «Тема н…» — резалось ДЕЙСТВИЕ, а не сообщение.
          Полный текст остаётся в `title` и в доступном имени. */}
      {error && (
