 ...
 *
 * ЧТО ДЕЛАЕТ ФРАЗУ ВЕРНОЙ — ЗАМЕР, А НЕ ДОГАДКА. На проде 08.09.2026, до
 * выката миграции, в `attachment` было НОЛЬ строк (замер целиком — в
 * `packages/db/drizzle/0089_attachment_artifacts.sql`, один источник на все
 * ссылки). То есть строк старше этой даты не существует вовсе, и обещание
 * экрана сегодня верно буквально; остаться верным оно может только так: всё
 * новое ложится ПОСЛЕ даты. Ложной фразу сделает появление вложений с
 * `created_at` раньше — например импорт исторических фото или чеков; тогда
 * либо переносить дату, либо снимать обещание с экрана. Прежнее обоснование
 * («полевые фото и чеки на проде старше даты есть, но документов бота нет»)
 * опиралось на оценку, а не на таблицу.
 *
