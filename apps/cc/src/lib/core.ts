 ...
 * минуту. Системная пауза задач состояние НЕ подменяет (перепроверка прода,
 * корень 1): занятый под ней агент `working`, остановленный Core — `blocked`,
 * и лишь свободному она даёт `paused` с причиной про настройку. Сам тумблер
 * едет в `paused.tasks`, и панель показывает его отдельной строкой.
 */
...
export interface AgentStatusRow {
  ...
  /**
   * Сколько ПОРУЧЕННЫХ задач агента ждут в очереди (`todo` без claim и затыка).
   *
   * Ожидание — не занятость, поэтому числом рядом с причиной, а не состоянием
   * (ревью Ф-1): под `AGENTS_TASKS_PAUSED=1` такая задача лежит до снятия
   * паузы, и плитка «молчит: последний прогон — выполнено» спорила со строкой
   * парка «новые порученные задачи никто не возьмёт». Ключа нет — очередь пуста.
   */
 ...
 * перечитывает настройки своим тиком (`AGENTS_SNAPSHOT_INTERVAL_MS`; об этом
 * честно говорит только `/system`: «применится в течение N минут») и кладёт
 * действующие у себя паузы в снимок расписаний. `lagging` — конфиг и снимок расходятся; `stale` —
...
export interface AppsHealthRow {
  ...
  at?: string;
  /**
   * Момент последней ПРОВЕРКИ источника (ISO) либо `null` — момента нет.
   *
   ...
   * ответ «ни разу».
   *
   * ИМЕННО `null`, А НЕ ОТСУТСТВИЕ ПОЛЯ (в отличие от соседей выше).
   ...
   *
   * ЧТО ЗНАЧИТ `null`, ОТВЕЧАЕТ `checksKnown` НИЖЕ, а не эта строка сама по
   * себе: смыслов у него два, и они противоположны.
   */
  lastCheckedAt: string | null;
  /**
   * Известно ли ядру, как обстоят дела с проверками (круг починок среза Д1, Ф-1).
   *
   * `true` + `null` — проверок не было НИ РАЗУ: экран пишет «не запускался», и
   * это утверждение Core, а не догадка панели. `false` + `null` — Core не смог
   * прочитать журнал проверок либо проходы были, а момента у них нет: экран
   * обязан сказать нейтральное «когда проверяли — неизвестно».
   *
   * ДО ЭТОГО ПОЛЯ ПАНЕЛЬ УТВЕРЖДАЛА ТО, ЧЕГО СИСТЕМА НЕ ЗНАЕТ. Диспетчер Notion
   * мог работать год — мы просто не смогли прочитать его таблицу, а строка
   * печатала «не запускался». Разбор — в `app/apps/page.tsx`, `времяСостояния`.
   */
  checksKnown: boolean;
  href?: string;
}
...
export const core = {
  ...
  /**
   * Свежесть факта кофе-заказов: сколько строк лежит и границы по времени.
   * `последний` — момент последнего заказа в базе. Импорт из панели
   * производителя ручной и может стоять днями — дашборд подписывает этим
   * кофе-зависимые плитки («данные до …»), иначе «Выручка · 30 дней» тихо
   * тает, маскируя стоящий импорт под спад продаж (аудит 02.09, блок A).
   */
  coffeeOrdersStatus: () =>
