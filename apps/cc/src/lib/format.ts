 ...
 *
 * НА ВХОДЕ ТОЛЬКО ISO-МОМЕНТ, и это требование к вызывающему, а не пожелание:
 * `new Date()` разбирает мусор по своим правилам, а не отказывается. Проверено
 * на настоящих входах: `ago("01.09.2026", …)` печатает «241 день назад» (V8
 * читает строку как 9 января), `ago(null as unknown as string, …)` — «20703 дня
 * назад» (null приводится к нулю эпохи). Прочерк отдаётся только на том, что
 * не разобралось ВОВСЕ. Отдельно для тех, кто будет печатать `lastCheckedAt:
 * string | null` из `GET /apps/health`: `null` — это «проверок не было», и
 * решать его обязан экран своим словом, а не подсовывать сюда.
 */
