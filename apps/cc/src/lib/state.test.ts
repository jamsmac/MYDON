 ...
 */
const ПОТРЕБИТЕЛИ = [
  { файл: path.join("components", "agent-grid.tsx"), берёт: ["AGENT_STATE_WORD", "AGENT_STATE_LED"] },
  { файл: path.join("app", "agents", "[name]", "page.tsx"), берёт: ["AGENT_STATE_WORD", "AGENT_STATE_LED"] },
...
describe("Слово и лампа меняются одним движением (срез Д1)", () => {
  ...
  it("экран не переписывает класс состояния от руки", () => {
    ...
    for (const { файл, берёт } of ПОТРЕБИТЕЛИ) {
      const код = текст(файл);
      for (const класс of запрещённыеКлассы(берёт)) {
