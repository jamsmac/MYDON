...
/**
 * Маршруты командного центра — ДАННЫЕ, а не проза (Р-Д2-3). ПРАВИЛО одно, но
 * список существует в двух копиях: здесь и в `rules.md` §4, где он напечатан
 * литералами, — автор нового экрана читает навык, а не модуль. Вторая копия
 * законна ровно потому, что под сторожем: `test/theme-routes.test.ts` сверяет
 * абзац §4 с этим массивом В ОБЕ СТОРОНЫ и роняет сборку на расхождении.
 * Это ПРЕФИКСЫ: `/agents` покрывает и карточку `/agents/[name]`.
 *
 * Новый маршрут стоит ДВУХ правок, а не одной: массив здесь и абзац §4 (три
 * зеркала навыка синхронны — их дословное равенство держит
 * `test/design-skill.test.ts`). Так добавится и `/artifacts` в срезе A3.
 */
