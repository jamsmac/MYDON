 ...
 *
 * Новый маршрут стоит ТРЁХ правок, а не двух: массив здесь, ТОЧНЫЙ СПИСОК в
 * `theme.test.ts` (он перечисляет префиксы второй раз намеренно — чтобы тест
 * падал, если из массива выпадет строка) и абзац §4 (три зеркала навыка
 * синхронны — их дословное равенство держит `test/design-skill.test.ts`).
 * Так и добавился `/artifacts` в срезе A3: измерено мутацией — правка только
 * массива роняет три сторожа в трёх файлах.
 */
export const CONSOLE_ROUTES: readonly string[] = [
  ...
  "/docs",
  "/apps",
  // Кольцо артефактов (срез A3): архив документов бота и полевых вложений —
  // тот же агентский слой, что `/flows` и `/apps`, и тот же тёмный мир.
  "/artifacts",
];
