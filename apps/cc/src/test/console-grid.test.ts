 ...
 *
 * `rules.md` §4: сетка ТОЛЬКО на холсте, НИКОГДА внутри панели; шаг 24px,
 * РАДИУС точки 1px (в поперечнике 2px: в `radial-gradient` единица у стоп-точки
 * это позиция стопа, а не диаметр), светлее фона на пару процентов; в светлом
 * мире её нет. jsdom стилей не применяет, поэтому сторожим исходник
 * `globals.css` тем же разборщиком, что сторож палитры (`test/css.ts`).
 *
...
describe("Сетка холста: какая она", () => {
  it("шаг 24px, радиус точки 1px, цвет — токен --line-soft, и обе ветки — один в один", () => {
    for (const ветка of ТЁМНЫЕ) {
