 ...
 *
 * МАРКЕР — HTML-комментарий `<!-- CONSOLE_ROUTES -->`, и он в §4 ровно один;
 * маршруты берутся из абзаца ПОД ним (строки до пустой строки, списка или
 * заголовка). Якорь — именно комментарий, а не проза: раньше тест цеплялся за
 * строку с подстрокой `` `CONSOLE_ROUTES` ``, HTML-комментарий не читал вовсе,
 * и обоснование соседнего сторожа («иначе тому тесту нечего вырезать»,
 * `design-skill.test.ts`) было ЛОЖНЫМ — маркер можно было унести в §1, оба
 * сторожа оставались зелёными, а следующий автор, правящий «абзац под
 * маркером», правил бы не тот абзац. Теперь маркер держит вырезку
 * по-настоящему: §4 можно переписывать, не трогая тест, — пока список стоит
 * под маркером, а прочие пути `/…` в обратных кавычках — в других абзацах.
 *
...
/** Абзац ПОД маркером: строки сразу за ним до пустой строки, списка или заголовка. */
function абзацМаркера(): string {
  ...
  expect(до, `сразу под ${МАРКЕР} в §4 нет ни одной строки абзаца`).toBeGreaterThan(от);
  return строки.slice(от, до).join("\n");
}

/**
 * Пути, где банальные строки «ConsoleTheme»/«console-theme» — не остаток
 * снесённого компонента, а сам текст сторожа, который их ищет.
 *
 ...
 * следа банальных строк «ConsoleTheme»/«console-theme» в СВОЁМ тексте.
 * Исключение — ИМЕННОЙ список, а не блок «все тесты»: каждому файлу в нём
 * приходится называть искомые строки текстом, чтобы их искать, — себе
 * (этот файл) и снесённому `console-theme.tsx` в `design-skill.test.ts`
 * (`rules.md` §4 ссылается на файл механизма по пути, и тест «навык не врёт
 * про код» обязан проверить, что путь `console-theme.tsx` НЕ существует —
 * см. `design-skill.test.ts`, «ручного штампа темы нет…»). Остальным файлам,
 * включая прочие тесты, прятаться незачем.
 */
const ИСКЛЮЧЕНИЯ = new Set([path.join("test", "theme-routes.test.ts"), path.join("test", "design-skill.test.ts")]);
...
function исходники(dir: string): string[] {
  ...
  for (const имя of readdirSync(dir)) {
    ...
    if (ИСКЛЮЧЕНИЯ.has(отн)) continue;
    out.push(отн);
  }
  return out;
}
...
describe("маршруты командного центра: модуль и навык не расходятся (Р-Д2-3)", () => {
  ...
  it("модуль знает решение владельца: командный центр тёмный по умолчанию", () => {
    ...
    // срез A3) сюда не добавлять — а когда придёт, добавится ДВУМЯ правками:
    // массив в `lib/theme.ts` и абзац §4 навыка. Запрет маршрута-призрака в
    // `design-skill.test.ts` для этого сделан условным, иначе корректная
    // работа A3 неизбежно дала бы красный.
...
describe("ConsoleTheme снесён, тему при навигации держит ThemeSync (Р-Д2-4)", () => {
  ...
  it("атрибуты ставит useLayoutEffect, а не useEffect", () => {
    // В jsdom эти два хука поведенчески НЕ РАЗЛИЧИТЬ: подмена на `useEffect`
    // оставляла весь набор зелёным (измерено ревью), а в браузере вернула бы
    // ровно тот кадр «новая страница в теме старой», против которого срез и
    // заведён. Поэтому сторож текстовый и стоит здесь, рядом с проверкой
    // `"use client"` по тому же исходнику. Докблок компонента заявляет это
    // гарантией — гарантия без сторожа тихо отменяется на полном гейте.
    //
    // Комментарии отброшены: докблок компонента сам называет слово `useEffect`,
    // объясняя, почему хук не он. Запрет ищется по ИМЕНИ, а не по вызову, — так
    // ловится и псевдоним (`useEffect as useLayoutEffect`).
    const код = текст(path.join("components", "theme-sync.tsx"))
      .replace(/\/\*[\s\S]*?\*\//g, "")
      .replace(/\/\/.*$/gm, "");
    expect(код, "эффект темы больше не useLayoutEffect").toContain("useLayoutEffect(");
    expect(код, "эффект темы переехал на useEffect — вернулся кадр в теме старой страницы").not.toMatch(
      /\buseEffect\b/,
    );
  });
});
...
/**
 * §9 навыка не называет снесённый механизм (круг починок 1, находка 2).
 *
 * ДО Д2 §4 и §9 врали СОГЛАСОВАННО — оба называли механизм темы через
 * `ConsoleTheme`. Задача 2 почистила §4 (абзац под маркером `CONSOLE_ROUTES`) и
 * код, а §9 оставила: та продолжала называть `<ConsoleTheme/>` со ссылкой на
 * снесённый `apps/cc/src/components/console-theme.tsx` и утверждать, что первый
 * кадр каждой тёмной страницы приходит светлым, — то есть переход от
 * согласованной лжи к рассогласованной, а он хуже честной непочинки. Сторож
 * выше (`исходники()`) этого не видел: он сканирует только `apps/cc/src`, а
 * `rules.md` живёт вне этого дерева.
 *
 * СТОРОЖ БЫЛ ЗАВЕДЁН РАНЬШЕ ПОЧИНКИ И БЫЛ КРАСНЫМ — НАМЕРЕННО: задача 2 писала
 * его, §9 не переписывая, чтобы забывчивость следующей задачи стала видимой, а
 * не молчаливой. Долг закрыт: два снятых абзаца §9 заменены одним, называющим
 * механизм (`dbf0eb7`), уборку §9 и §4 закончила задача 5. Разбор, ПОЧЕМУ
 * прежний сторож Д1 (он требовал обратного — чтобы §9 перечисляла расхождения
 * маршрутного списка) развёрнут, а не снят, лежит там же, где стоит сам
 * развёрнутый сторож: `design-skill.test.ts`, перед describe «rules.md §9».
 */
