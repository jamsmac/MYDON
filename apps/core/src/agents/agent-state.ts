 ...
 * карточке: рантайм перечитывает карточки раз в десять минут, и выключенный
 * владельцем агент до перечитки берёт задачи и жжёт cron (ревью I-1) — «на
 * паузе» над таким claim врало бы в обратную сторону. Такой агент —
...
export function computeAgentState(input: AgentStateInput): AgentStateVerdict {
  ...
  if (live !== null) {
    ...
    if (blocked !== null && (blocked.task.id === live.task.id || blocked.at.getTime() >= live.at.getTime())) {
      return затык(blocked.task, blocked.at);
    }
    ...
    const хвосты = [
      ...
      паспортВыключен
        ? `в карточке выключен (статус ${input.passportStatus}) — дорабатывает начатую задачу, новых не возьмёт`
        : null,
  ...
  if (blocked !== null) return затык(blocked.task, blocked.at);
...
function затык(task: ClaimedTaskLite, at: Date): AgentStateVerdict {
  return {
    state: "blocked",
