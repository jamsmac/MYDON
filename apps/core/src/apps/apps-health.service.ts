...
export class AppsHealthService {
  ...
  async health(now = new Date()): Promise<AppsHealthView> {
     ...
     * нет» или «журнал не прочитан», а для сбора ещё и отчёт OurVend, — и
     * дверь отвечает так же, как ответила бы правилу на полном входе. Свой
     * ответ у службы был (`проверкаПоЖурналу`), считанный по одному журналу,
     * и при отказе снимка он печатал «не запускался» над отчётом с двадцатью
     * прогонами, который лежал в `ourvend.value` и не спрашивался.
     *
  ...
  private строкаУчёта(
    ...
    const прогон = прочитанныйПрогон(face.key);
    // У отчёта учёта моментов нет вовсе, поэтому свидетельство одно — тик
    // монитора, и дверь та же, что у обычного монитора и у правила
    // `rowFromOurvendAccounting`.
    const проверено = сведенияОПроверкеМонитора(прогон);
    if (базаОтказала !== null) return unavailableRow(face, базаОтказала, проверено);
    if (!ourvend.ok) return unavailableRow(face, ourvend.источник, проверено);
