...
export class AppsHealthService {
  ...
  async health(now = new Date()): Promise<AppsHealthView> {
     ...
     * причине отказа: журнал не прочитан — «не знаем»; прочитан и пуст — ноль
     * свидетельств, и только это утверждение экран печатает словом.
     *
     * НОЛЬ ЗДЕСЬ — ПЕРЕСЧЁТ, А НЕ ФОЛБЭК: журнал прочитан целиком, прогона
     * этого монитора в нём нет. Нечитаемые прогоны отброшены раньше, на
     * границе (`последниеПрогоны`), поэтому «строка есть, а момент мусор» сюда
     * не доезжает — иначе ноль был бы неправдой.
     */
    const прогоныНеЧитаны = отказ(прогоны) !== null;
    const проверкаПоЖурналу = (lastRun: MonitorRunLite | null): СведенияОПроверке =>
      прогоныНеЧитаны ? "не знаем" : (lastRun?.at ?? { свидетельств: 0 });
