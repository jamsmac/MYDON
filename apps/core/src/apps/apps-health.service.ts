...
/**
 * Сколько ждём ОДИН источник, прежде чем считать его непрочитанным (П3).
 *
 * ГЛАВНОЕ СВОЙСТВО ЧИСЛА — ОНО МЕНЬШЕ, ЧЕМ РЕЖЕТ ПАНЕЛЬ: та рвёт запрос на
 * восьмой секунде (`AbortSignal.timeout(8000)`, `apps/cc/src/lib/core.ts`).
 * `Promise.all` ловил только отказ, таймаута на источник не было вовсе — и
 * один медленный источник (OurVend после истечения минутного кеша, ledger на
 * нагруженной базе — 9–10 с) уносил ВЕСЬ `/apps` в `CoreDown`: восемь готовых
 * строк исчезали с экрана, а ярлык «не прочитано» не появлялся никогда, потому
 * что ответа не было вообще. Пять секунд оставляют панели три секунды на
 * дорогу и разбор.
 *
 * ЧЕГО ЭТОТ ТАЙМАУТ НЕ ДЕЛАЕТ — НЕ ОТМЕНЯЕТ ЗАПРОС. Nest зависшее чтение не
 * прервёт: оно доработает в фоне, а его результат будет отброшен (соединение
 * СУБД освободится, когда драйвер сам закончит). Это известно и принято: цель
 * не в том, чтобы отменить один запрос, а в том, чтобы из-за него не потерять
 * восемь готовых строк.
 */
const ТАЙМАУТ_ИСТОЧНИКА_МС = 5_000;

/**
 * Метка исхода «не дождались» — ЗНАЧЕНИЕМ, а не исключением.
 *
 * «Источник не ответил за N с» — не ошибка чтения, а решение перестать ждать,
 * и в `catch` ему делать нечего: там разбирают ПРИЧИНУ отказа, которой здесь
 * нет. Символ, а не строка: строку мог бы вернуть и сам источник.
 */
const НЕ_ДОЖДАЛИСЬ = Symbol("источник не ответил за таймаут");

/** Мс в секунды по-русски: 5000 → «5», 200 → «0,2». Число в ярлыке — из константы. */
function секундами(мс: number): string {
  return (мс / 1000).toLocaleString("ru-RU");
}

/** Причина отказа словами — одним разбором на все три места, где она пишется в журнал. */
function причинаСловами(e: unknown): string {
...
export class AppsHealthService {
  ...
  /**
   * Таймаут одного источника — ПОЛЕ, а не константа в теле `попытка()`.
   *
   * Ассерт на этот таймаут обязан быть, а ждать в наборе тестов пять секунд —
   * значит сделать набор медленным на ровном месте: наследник подменяет поле и
   * проверяет РОВНО ТОТ `попытка()`, что уедет на прод.
   *
   * ПОЧЕМУ НЕ ЧИСЛОВОЙ ПАРАМЕТР КОНСТРУКТОРА — ПРОВЕРЕНО, А НЕ ПРЕДСКАЗАНО
   * (`@nestjs/core` 11.1.28; круг починок 1, находка 1). Класс собирает Nest с
   * `emitDecoratorMetadata`, поэтому примитив попадает в `design:paramtypes` и
   * разрешается как зависимость: БЕЗ `@Optional()` — `UnknownDependenciesException`
   * при загрузке модуля (`injector.js`: `if (!isOptional) throw err`), С
   * `@Optional()` — `undefined`, то есть срабатывает значение по умолчанию и
   * таймаут остаётся боевым. Тихого нуля не даёт ни одна ветка (первая
   * редакция этого комментария такой исход ПРЕДСКАЗЫВАЛА — неверно). Поле
   * выбрано не из-за него: оно не требует декоратора ради теста и держит
   * проверку на том же пути, что работает на проде.
   */
  protected readonly таймаутИсточникаМс: number = ТАЙМАУТ_ИСТОЧНИКА_МС;
  ...
  async health(now = new Date()): Promise<AppsHealthView> {
    ...
    const rows: HealthRow[] = [
      ...
      расписания.ok
        ...
        // И ДВЕРИ СКАЗАНО СЛОВОМ, ЧТО СНИМОК НЕ ЧИТАЛСЯ (П5). `свежесть` на
        // этой ветке `null` по построению — она считается только при
        // `расписания.ok`, — но у `null` два ПРОТИВОПОЛОЖНЫХ смысла: «снимка
        // нет» у правила и «снимок не прочитан» здесь. Пока второй аргумент не
        // спрашивали, дверь отвечала «не знаем» обоим, и строка над ПУСТЫМ
        // снимком печатала «когда проверяли — неизвестно» над собственным
        // вердиктом «агенты ещё не отчитывались».
        : unavailableRow(FACES.agents, расписания.источник, сведенияОПроверкеСлоя(свежесть, false)),
      ...
      //
      // ИСКЛЮЧЕНИЕ — ИМЕНА ЗАНЯТЫХ КЛЮЧЕЙ, И ЭТО ОСОЗНАННЫЙ ОБМЕН (M-8):
      // монитор с именем `agents` строки действительно не получит. Дубль
      // React-key и два вердикта об одном ключе хуже, чем потеря записи, о
      // которой на том же экране уже говорит строка слоя.
      ...[...мониторы.keys()]
  ...
  /**
   * Чтение источника под своим `catch` И СВОИМ ТАЙМАУТОМ: и отказ, и молчание
   * становятся значением, а не судьбой всего ответа. В строку панели едет
   * ЯРЛЫК прочитанного, а причина — только в журнал Core: маршрут читается
   * анонимно, а сообщения драйвера несут хост и пользователя базы.
   *
   * ТАЙМАУТ ЗДЕСЬ, А НЕ ВОКРУГ `Promise.all` (П3). Общий таймаут на все шесть
   * чтений отдал бы ту же пустоту, что и её отсутствие: строк восемь, а
   * причина одна. Гонка внутри одной попытки делает молчание источника ЕГО
   * строкой — той же `unavailableRow`, что и отказ, только с ярлыком, который
   * называет секунды из константы.
   */
  private async попытка<T>(что: string, fn: () => Promise<T>): Promise<Чтение<T>> {
    const мс = this.таймаутИсточникаМс;
    let таймер: ReturnType<typeof setTimeout> | undefined;
    let сдались = false;
    const чтение = fn();
    /*
     * ПРИЧИНА ПОЗДНЕГО ОТКАЗА ВСЁ РАВНО ЕДЕТ В ЖУРНАЛ (круг починок 1, находка
     * 7). Источник, который стабильно отвечает 9–10 с и ПОТОМ падает, — ровно
     * тот случай, ради которого таймаут и заведён; без этой строки в журнале
     * оставалось бы только «ответа нет за 5000 мс», а сообщение драйвера не
     * писалось бы никогда. Диагностика мотивирующего сценария не имеет права
     * быть беднее прежней, когда `попытка()` ждала до конца и причину знала.
     *
     * Только ПОСЛЕ того, как сдались: ранний отказ разбирает `catch` ниже, он
     * же пишет причину, и вторая запись о том же событии — шум. Обработчик
     * навешен ДО гонки, поэтому на раннем отказе он отработает первым, когда
     * `сдались` ещё `false`.
     *
     * Наружу, в строку панели, это не едет: строка уже сказала правду —
     * источник не ответил за N с, — а сообщения драйвера несут хост и
     * пользователя базы (постановление ветки).
     */
    чтение.catch((e: unknown) => {
      if (!сдались) return;
      this.logger.warn(`${что} не прочитан: источник ответил отказом уже после таймаута: ${причинаСловами(e)}`);
    });
    try {
      // Отброшенный результат никого не роняет: обработчики висят на обоих
      // участниках гонки (и свой `catch` выше), поэтому поздний отказ зависшего
      // чтения остаётся обработанным, а не уносит процесс `unhandledRejection`.
      const итог = await Promise.race([
        чтение.then((value) => ({ value })),
        new Promise<typeof НЕ_ДОЖДАЛИСЬ>((resolve) => {
          таймер = setTimeout(() => resolve(НЕ_ДОЖДАЛИСЬ), мс);
        }),
      ]);
      if (итог === НЕ_ДОЖДАЛИСЬ) {
        сдались = true;
        this.logger.warn(`${что} не прочитан: ответа нет за ${мс} мс`);
        return { ok: false, источник: `${что} — источник не ответил за ${секундами(мс)} с` };
      }
      return { ok: true, value: итог.value };
    } catch (e) {
      this.logger.warn(`${что} не прочитан: ${причинаСловами(e)}`);
      return { ok: false, источник: что };
    } finally {
      // Таймер гасим ВСЕГДА: живой пятисекундный `setTimeout` на каждый из
      // шести источников держал бы событийный цикл и после ответа — на проде
      // это утечка, а в наборе тестов длительность прыгает с 0,6 до 5,6 с.
      clearTimeout(таймер);
    }
  }
}
...
/**
 * Ключи, под которыми на витрине УЖЕ СТОИТ СТРОКА: монитор с таким именем
 * своей строки не получает, остальные идут во «внутренние» под своим именем.
 *
 * ЛИЦО СЛОЯ АГЕНТОВ В НАБОРЕ — ЗАЩИТА, А НЕ ПОЧИНКА (M-8). Строку слоя
 * собирает свежесть снимка, а НЕ запись снимка: у монитора с именем `agents`
 * своей строки нет вовсе, поэтому без этого ключа он получил бы строку с уже
 * занятым `key` — дубль React-key на `/apps` и два вердикта об одном ключе. На
 * сегодняшних именах мониторов (`x:y`) вход недостижим, но имя приезжает из
 * тела `PUT /routines/snapshot`, то есть его выбирает рантайм, а не этот файл.
 */
