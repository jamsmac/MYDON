...
export class AppsHealthService {
   ...
   * значит сделать набор медленным на ровном месте: тест подменяет поле
   * наследником. Числовым параметром конструктора его сюда не внести — этот
   * класс собирает Nest, и лишний примитив в `design:paramtypes` он попытается
   * разрешить как зависимость.
   */
  ...
  private async попытка<T>(что: string, fn: () => Promise<T>): Promise<Чтение<T>> {
    ...
    try {
      // Отброшенный результат никого не роняет: обработчики `Promise.race`
      // висят на обоих участниках, поэтому поздний отказ зависшего чтения
      // остаётся обработанным, а не уносит процесс `unhandledRejection`.
      ...
      // шести источников держал бы событийный цикл и после ответа.
      clearTimeout(таймер);
    }
  }
}
...
/**
 * Ключи, у которых есть своё лицо: монитор с таким именем второй строки не
 * получает — остальные идут во «внутренние» под своим именем.
 *
 * ЛИЦО СЛОЯ АГЕНТОВ В НАБОРЕ — ЗАЩИТА, А НЕ ПОЧИНКА (M-8). Его строку собирает
 * не снимок, а свежесть снимка, поэтому монитор с именем `agents` дал бы ВТОРУЮ
 * строку с тем же `key` — дубль React-key на `/apps` и два вердикта об одном
 * ключе. На сегодняшних именах мониторов (`x:y`) это недостижимо, но имя
 * приезжает из тела `PUT /routines/snapshot`, то есть его выбирает рантайм, а
 * не этот файл.
 */
