...
describe("Здоровье приложений: слой агентов по возрасту снимка (перепроверка прода, корень 2)", () => {
  it("снимка нет — «не оценить», а не «сломано»: слой мог ни разу не запуститься", () => {
    ...
    // НОЛЬ ДОКАЗУЕМ, И ПОТОМУ ЭТО УТВЕРЖДЕНИЕ (ревью слияния A2 и Д1, П5).
    // Проверка ЭТОЙ строки определена как собственный отчёт слоя, и другого
    // мерила у неё нет: прочитанная пустая `agent_runtime_snapshot` доказывает
    // ноль отчётов, то есть ноль проверок. Прежнее «не знаем» спорило с
    // вердиктом той же строки: колонка печатала «когда проверяли —
    // неизвестно» над «агенты ещё не отчитывались».
    assert.equal(row.lastCheckedAt, null);
    assert.equal(row.checksKnown, true, "снимок прочитан и пуст — отчётов ноль, и это доказано");
  });

  it("ДВЕРЬ СВИДЕТЕЛЬСТВ СЛОЯ РАЗЛИЧАЕТ «снимка нет» и «снимок не прочитан» (П5)", () => {
    // Тот же `null` на входе, ПРОТИВОПОЛОЖНЫЙ ответ на выходе — и это ровно то,
    // чего дверь не умела, пока второй аргумент не спрашивали: у правила `null`
    // значит «снимка нет», у службы на ветке отказа — «снимок не прочитан».
    assert.deepEqual(сведенияОПроверкеСлоя(null, true), { свидетельств: 0 });
    assert.equal(сведенияОПроверкеСлоя(null, false), "не знаем");
    // Снимок есть — ответ от `снимокПрочитан` не зависит: момент отчёта и есть
    // ответ, а мусорный момент — одно свидетельство.
    const момент = new Date(NOW.getTime() - 5 * 60_000);
    for (const прочитан of [true, false]) {
      assert.equal(
        сведенияОПроверкеСлоя({ ageSec: 300, stale: false, reportedAt: момент }, прочитан),
        момент,
      );
      assert.deepEqual(
        сведенияОПроверкеСлоя({ ageSec: Number.NaN, stale: false, reportedAt: new Date("сломано") }, прочитан),
        { свидетельств: 1 },
      );
    }
  });
