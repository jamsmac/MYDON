...
/**
 * ЧТО ИЗВЕСТНО О ПРОВЕРКАХ ИСТОЧНИКА — три ответа, а не два (Ф-1).
 *
 * `Date` — проверка была вот тогда. `"ни разу"` — журнал проверок прочитан, и
 * проверок в нём нет: это утверждение, по нему экран пишет «не запускался».
 * `"не знаем"` — прочитать не удалось либо факт проверки есть, а момента нет;
 * «не запускался» здесь было бы ложью об источнике, который мог работать год.
 *
 * СОЮЗ, А НЕ `Date | null`. `null` нёс оба отрицательных смысла сразу, и
 * различить их на экране было нечем. Теперь каждый вызывающий обязан ответить
 * СЛОВОМ, какой у него случай, — промолчать в этом месте больше нельзя.
 */
export type СведенияОПроверке = Date | "ни разу" | "не знаем";
...
function сПроверкой(строка: HealthRow, проверка: СведенияОПроверке): HealthRow {
  // «Не знаем» — это БАЗА строки (`row`), поэтому здесь ей нечего добавить.
  if (проверка === "не знаем") return строка;
  // НЕЧИТАЕМЫЙ МОМЕНТ ПРИРАВНЕН К ОТСУТСТВУЮЩЕМУ — единое правило файла
  // (`читаемыйМомент`), и «не знаем» его НЕ отменяет: вердикт по такому входу
  // уже сказал «прогонов не было» (`сЧитаемымПрогоном` обнулил прогон ДО
  // правил), и «когда проверяли — неизвестно» в колонке времени спорило бы с
  // собственным заголовком строки — тот самый дефект, который чинит Ф-1.
  const момент = проверка === "ни разу" ? null : исо(проверка);
  return { ...строка, lastCheckedAt: момент, checksKnown: true };
}
...
export function rowFromMonitor(face: FaceMeta, input: MonitorRowInput): HealthRow {
  ...
  return безЗелёногоНаБитомМоменте(
    сПроверкой(вердиктМонитора(face, вход), вход.lastRun?.at ?? "ни разу"),
...
export function rowFromOurvendSync(face: FaceMeta, input: OurvendSyncInput): HealthRow {
  ...
  return сПроверкой(вердиктСбора(face, вход), проверкаСбора(вход) ?? "ни разу");
}
...
export function rowFromOurvendAccounting(face: FaceMeta, input: OurvendAccountingInput): HealthRow {
  const вход = сЧитаемымПрогоном(сЧитаемымПорогом(input));
  return безЗелёногоНаБитомМоменте(
    сПроверкой(вердиктУчёта(face, вход), вход.lastRun?.at ?? "ни разу"),
    битыйМомент(input.silentAfter) ? ПОРОГ_МОЛЧАНИЯ_НЕ_ЧИТАЕТСЯ : null,
  );
}
...
export function rowFromHeartbeat(face: FaceMeta, input: HeartbeatRowInput): HealthRow {
  ...
  return сПроверкой(вердиктСигнала(face, { ...input, lastAt }), lastAt ?? "ни разу");
}
 ...
 *
 * ДВА РАЗНЫХ «МОМЕНТА НЕТ», И ОНИ РАЗВЕДЕНЫ (Ф-1). Монитор ledger не ответил
 * (`monitoring === null`) — о вызовах не известно НИЧЕГО: «не запускался» врало
 * бы о маршруте, который мог отработать минуту назад. Монитор ответил, а
 * завершённых вызовов в нём нет — это «ни разу», и вот его экран печатает
 * словом.
 */
export function rowFromLlm(face: FaceMeta, input: LlmRowInput): HealthRow {
  return сПроверкой(
    вердиктМоделей(face, input),
    input.monitoring === null ? "не знаем" : (моментВызова(input.monitoring) ?? "ни разу"),
  );
}
 ...
 *
 * - закрытых доставок нет вовсе — диспетчер ни разу не ходил: «ни разу»;
...
function проверкаДоставок(input: OutboxRowInput): СведенияОПроверке {
  ...
  return закрытыхДоставок(input) > 0 ? "не знаем" : "ни разу";
}
