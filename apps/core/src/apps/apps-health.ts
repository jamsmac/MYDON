...
export function rowFromMonitor(face: FaceMeta, input: MonitorRowInput): HealthRow {
  ...
  return безЗелёногоНаБитомМоменте(
    сПроверкой(вердиктМонитора(face, вход), вход.lastRun?.at ?? свидетельстваПрогона(input.lastRun)),
...
export function rowFromOurvendSync(face: FaceMeta, input: OurvendSyncInput): HealthRow {
  ...
  return сПроверкой(вердиктСбора(face, вход), проверкаСбора(вход) ?? свидетельстваСбора(input));
}

/**
 * Свидетельства проверок сбора, когда читаемого момента нет ни одного.
 *
 * ДВЕ РАЗНЫЕ ТАБЛИЦЫ, И ЛЮБОЙ ОДНОЙ ДОСТАТОЧНО. `health.runs` считается по
 * `vending_sync_run` (`ourvend-health.service.ts`), а `lastRun` — по
 * `agent_run`. Коллектор ОТКРЫВАЕТ строку `vending_sync_run` в НАЧАЛЕ прогона
 * (`ourvend-sync.ts`), а `agent_run` пишет колбэк монитора ПОСЛЕ и best effort
 * (`monitors.ts`), поэтому «в журнале прогонов пусто» — ещё не «сбор не
 ...
 *
 * ОТЧЁТ НЕ СОБРАЛСЯ — ПЕРЕСЧИТЫВАТЬ НЕЧЕМ, и это «не знаем»: утверждать «сбор
 * не запускался» по одному пустому `agent_run` мы права не имеем.
 */
function свидетельстваСбора(input: OurvendSyncInput): СведенияОПроверке {
  if (input.health === null) return "не знаем";
  return {
    свидетельств: Math.max(0, input.health.runs) + (input.lastRun === null ? 0 : 1),
  };
}
...
export function rowFromOurvendAccounting(face: FaceMeta, input: OurvendAccountingInput): HealthRow {
  ...
  return безЗелёногоНаБитомМоменте(
    ...
    сПроверкой(вердиктУчёта(face, вход), вход.lastRun?.at ?? свидетельстваПрогона(input.lastRun)),
    битыйМомент(input.silentAfter) ? ПОРОГ_МОЛЧАНИЯ_НЕ_ЧИТАЕТСЯ : null,
  );
}
