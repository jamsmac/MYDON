...
export function журнальноеДействие(from: string, to: string): string {
  ...
  if (from >= ОКНО_СРАВНЕНИЯ.с && to <= ОКНО_СРАВНЕНИЯ.по) {
    return (
      `за окно ${ОКНО_СВЕРЕНО.с}…${ОКНО_СВЕРЕНО.по} записи не велись ни в одной системе (VendCash сверен ${ДОНОР_СВЕРЕН}: те же 386 инкассаций, ` +
...
export class GapsService {
   ...
   */
  async list(today: string = dayKey(new Date(), TZ)): Promise<Gap[]> {
    // «Сегодня» — параметр, а не вызов часов внутри: детекторы тишины считают
    // дни от него, и тест с фикстурой на фиксированную дату иначе начинал
    // падать сам по себе спустя две недели после написания (06.09.2026).
