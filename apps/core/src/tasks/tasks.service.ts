...
export class TasksService {
  ...
  async create(input: CreateTaskInput, actorRef = "system"): Promise<TaskRow> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      // отстаёт на период `AGENTS_SNAPSHOT_INTERVAL_MS` — числом здесь стояло
      // «10 минут», пережившее переход на пятиминутный тик) и доверяет Core —
      // значит, проверять обязан Core,
