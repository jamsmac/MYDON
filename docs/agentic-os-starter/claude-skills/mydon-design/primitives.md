...
| `.approw[data-kind]` — СТРОКА АРТЕФАКТА | второй жилец `.approw` (срез A3, архив `/artifacts`): та же анатомия, другое содержание — лампа `.led` со словом типа (`ARTIFACT_KIND_WORD[kind]` / класс `ARTIFACT_KIND_LED[kind]`, `apps/cc/src/lib/state.ts`; тип мимо словаря печатается как есть с базовой `.led`) + `.ab` (`.an` — НАЗВАНИЕ-ССЫЛКА на файл через прокси `/api/attachments/:id/raw`; `.as` — автор · владелец-ссылка · направление) + `.aw` — давность `ago(createdAt, now)`, где `now` — часы Core из ответа `GET /artifacts`, а не браузера. Своего класса НЕ завёл: `.approw` уже описывает эту геометрию, а второй класс с той же вёрсткой разъехался бы с первым | `app/artifacts/page.tsx` (`div.approw` — строка НЕ ссылка целиком: внутри две ссылки, вложенные `<a>` браузер разорвал бы сам) | `data-kind` — ЯРЛЫК ДЛЯ ГЛАЗА, не ось состояния: полос по нему нет (`data-state` полосы `--err`/пунктир остаётся у `/apps`, §4.2). Пустой архив — `.empty` с датой начала архива (`ARTIFACTS_SINCE`, `apps/cc/src/lib/artifacts.ts`), а не «ничего не найдено»: документы до среза были, их выбрасывали |
