...
**Цена, которую платит владелец по ЗАДЕРЖКЕ (осознанный обмен, не дефект).** Порядок «архив →
Telegram» ставит перед отправкой два сетевых шага, и оба ждут ответа: `personOf` (`/people/by-chat`,
общий таймаут клиента 10 с) и `uploadDocument` (`UPLOAD_TIMEOUT_MS = 60 с`,
`apps/bot/src/core-client.ts`). До A3 файл уходил в чат сразу после генерации; теперь при
подвисшем Core он придёт позже — в худшем случае на ~70 с, из них 60 приходится на загрузку. Файл
при этом НЕ теряется и владельцу говорят словами («Файл отправлен, но в архив не лёг: Core не
ответил вовремя»), поэтому обмен принят: минута ожидания при больном ядре против потерянного
навсегда документа при сорвавшейся отправке. Обычный случай задержки не даёт — POST идёт внутри
docker-сети, файл ≤ 12 МБ (предел `FileInterceptor`).

Число 60 с здесь НЕ выбиралось под документ: `UPLOAD_TIMEOUT_MS` заведён для `uploadPhoto`
(полевой контур, где никто не ждёт доставки) и переиспользован. Для документа он великоват — 12 МБ
по docker-сети даже при 2 МБ/с это ~6 с, — но уменьшать общую константу нельзя, не разделив её на
две: у фото своя цена ошибки. Отдельный (меньший) таймаут для пути документа — кандидат
следующего среза, а не правка «заодно» в этом.
