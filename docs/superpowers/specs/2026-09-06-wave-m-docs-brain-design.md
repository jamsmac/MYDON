...
| R-M-4 узлы навыков | файл `agents/*/skills/*.md` = узел вида `skill` (id = путь, `href /skills`); синтетический `skill:<a>/<s>` только без файла; ребро `describes` агент → ROLE.md | Иначе один навык давал два узла без связи |
...
| Р-4 `marked` без сырого HTML | + `text()` override: после незакрытого строчного `<code>/<pre>/<kbd>/<script>` marked отдаёт текст неэкранированным (`inRawBlock`) — воспроизведено на 18.0.11 и закрыто; `escapeText` не дублирует уже экранированные мнемоники | XSS-угол, который тесты с `<script>` не щупали |
...
| §7 приёмка | локально: дерево 208, граф 219 узлов / 522 ребра; smoke 23/23 на Postgres 15 | Прод — после мержа |
