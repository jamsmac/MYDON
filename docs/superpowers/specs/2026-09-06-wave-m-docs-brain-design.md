...
| R-M-4 рёбра `links` | + `mentions` (пути в обратных кавычках → существующие файлы белого списка): на момент реализации 478 рёбер против 23 `links` на 208 файлах (после исключения копий стартера и `_template` — 203 файла, ~221 узел, ~560 рёбер) | Репозиторий пишет пути в бэктиках, markdown-ссылок почти нет — граф без `mentions` был бы россыпью изолированных узлов |
