...
async function проверитьЛица() {
  ...
  const состояние = async () => {
    ...
    // Д-3: намерение против факта — рантайм перечитывает тумблеры раз в 5 минут
    // (`AGENTS_SNAPSHOT_INTERVAL_MS`); до перехода на пятиминутный тик тут было 10.
...
async function проверитьЗдоровьеПриложений() {
  ...
  // три тика по 5 минут — два пропущенных прощаются, ревью I-3): все
