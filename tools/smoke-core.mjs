...
async function проверитьЛица() {
  ...
  try {
    ...
    // А без занятости та же пауза — «на паузе» с причиной про настройку:
    // задача закрывается, и об агенте сказать больше нечего.
    ...
    if (свободен.state !== "paused" || !/настройка системы, а не агента/.test(свободен.reason)) {
      throw new Error(`без занятости при системной паузе: ${свободен.state} — «${свободен.reason}»`);
    }
    if (!/cron-прогоны этой паузой не остановлены/.test(свободен.reason)) {
      throw new Error(`причина паузы не говорит, что cron идёт: «${свободен.reason}»`);
    }
