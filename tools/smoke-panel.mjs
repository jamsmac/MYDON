...
const СТРАНИЦЫ = [
  ...
  // У «Прогонов» слово пока из каркаса: страницу дописывает соседний срез, и
  // фразы её тела ещё нет. Значит эта строка проверяет ДОСТАВКУ (маршрут есть,
  // SSR не упал в 500, тело не пустое); заменить на слово из тела — когда
  // страница ляжет.
  { path: "/flows", должно: "Прогоны" },
