...
const СТРАНИЦЫ = [
  ...
  // Волна R. У «Рутин» слово берём из ТЕЛА страницы: подпись тумблера паузы
  // рисуется, только когда доска доехала из Core. «Рутины» здесь не годятся —
  // это пункт меню, то есть каркас, и проверка прошла бы на экране ошибки.
  { path: "/crons", должно: "Расписания cron" },
  // У «Прогонов» слово из ТЕЛА страницы — пункт формы фильтров. Форма рисуется
  // при ЛЮБОМ состоянии журнала (и на пустом, и на отфильтрованном), поэтому
  // проверка не зависит от данных; заголовок «Прогоны» здесь не годится — он
  // же стоит в меню, то есть в каркасе, и нашёлся бы на экране ошибки.
  { path: "/flows", должно: "Любой исход" },
  // Волна A2. Слово берём из ТЕЛА страницы — из заголовка раздела «Внутренние
  // мониторы», который рисуется при ЛЮБОМ ответе Core (в том числе пустом:
  // третье состояние — это строка, а не исчезнувший раздел). «Приложения»
  // здесь не годятся — это пункт меню, то есть каркас, и проверка прошла бы на
  // экране ошибки.
  { path: "/apps", должно: "Внутренние мониторы" },
