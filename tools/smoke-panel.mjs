...
const СТРАНИЦЫ = [
  ...
  // Волна R: доска рутин и плейбэк прогонов. Слова «Рутины» и «Прогоны» есть и
  // в пунктах меню, то есть в каркасе, — здесь они проверяют ДОСТАВКУ страницы
  // (маршрут есть, SSR не упал в 500, тело не пустое). Содержимое доски и
  // плейбэка закрыто тестами страниц в cc и сценарием «рутины» в smoke-core:
  // там оно проверяется по данным, а не по подписи.
  { path: "/crons", должно: "Рутины" },
